...
class MMU:
    ...
    def reset(self):
        ...
        for b in self.blocks:
            if not b['readonly'] and b['initialize']:
                b['memory'] = array.array('B', [0]*b['length'])

    def addBlock(self, start, length, readonly=False, value=None, valueOffset=0, blockfunc = None):
        """
        ...
        if blockfunc == "ROM":
            newBlock = {
                ...
                'memory': array.array('B', [0]*length), 'blockfunc': None, "initialize": False
            }
        else:
            newBlock = {
                'start': start, 'length': length, 'readonly': readonly,
                'memory': array.array('B', [0]*length), 'blockfunc': blockfunc, "initialize": True
            }
    ...
    def write(self, addr, value):
        ...
        # If function is define, call the function with the block, address, index, value, and read flag
        if b['blockfunc'] is not None:
            b['blockfunc'](b, addr, i, value, False)
    ...
    def read(self, addr):
        ...
        # If function, return a value with block, address, index, value, and read flag
        if b['blockfunc'] is not None:
            return b['blockfunc'](b, addr, i, 0, True)
        return b['memory'][i]
